...
func flagSet() *flag.FlagSet {
	...
	fs.String("channel", "nsq_to_filebeat", "nsq channel")
...
func autoClear(dataPath string){
	time.AfterFunc(5*time.Minute, func() {
        autoClear(dataPath)
    })

	var dirSize int64
    flist, e := ioutil.ReadDir(dataPath)
    if e != nil {
        log.Printf("read file %s error", dataPath)
        return
    }
    ...
    for _, f := range flist {
        if f.IsDir() == false{
			...
			if dirSize > 1024000000{
				filePath := fmt.Sprintf("%s/%s", dataPath, f.Name())
				log.Printf("delete file is %s", filePath)
				os.Remove(filePath)
			}
        }
    }
	log.Printf("data %s total size %d", dataPath, dirSize)
}

func reverse(input []fs.FileInfo) []fs.FileInfo {
    if len(input) == 0 {
        return input
    }
